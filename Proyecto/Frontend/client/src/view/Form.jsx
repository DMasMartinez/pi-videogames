...
const Form =()=>{
    const deviceslist = ["PC","PS1","PS2","PS3","PS4","PS5","XBOX","XBOX360","XBOXSERIESX","XBOXSERIESS"]
    ...
    const [game,setGame]= useState({
        ...
        description:"",
        devices:[],
        release:fecha.dia&&fecha.mes&&fecha.ano&&`${fecha.dia}-${fecha.mes}-${fecha.ano}`,
        image:pacman,
        ratings:"",
    ...
    function handlerchange(event){
        ...
        if (event.target.name==="devices"){
            if (event.target.checked===true){
                ...
                setError(Validation({...game,devices:[...plataform,event.target.value]}))
            }else{
                setPlataform([...plataform.filter((device)=>device!=event.target.value)])
                setGame({...game,devices:[...plataform.filter((device)=>device!=event.target.value)]})
                setError(Validation({...game,devices:[...plataform.filter((device)=>device!=event.target.value)]}))
            }
    ...
    return (
        <div class="form-container">
            <form onSubmit={handleSubmit}>
                ...
                <label class="form-label" htmlFor="devices">devices: 
                    {deviceslist.map((device,index)=>{
                        return (
                            <div className="checklist2" key={index}>
                                <label>
                                    <input name="devices" value={device} type="checkbox" onChange={handlerchange}/>
                                    {device}
                                </label>
                            </div>
                        )
                ...
                <div className="fecha">
                    <label class="form-label" htmlFor="day">day: </label>
                    <input class="form-input" name="day" value={fecha.dia} onChange={handlerchange} cols={10}/>

                    <label class="form-label" htmlFor="month">month: </label>
                    <input class="form-input" name="month" value={fecha.mes} onChange={handlerchange} cols={10}/>

                    <label class="form-label" htmlFor="year">year: </label>
                    <input class="form-input" name="year" value={fecha.ano} onChange={handlerchange} cols={10}/>
                </div>
                ...
                {game.name!=""&&game.description!=""&&game.release!=""&&game.devices!=""&&game.ratings!=""&&game.Genres!=""&&Object.keys(error).length===0&&Object.keys(errorfecha).length===0?<button class="form-button" type="Submit">submit</button>:<button class="form-button-disabled" disabled type="Submit">submit</button>}
            </form>
        </div>
    )
