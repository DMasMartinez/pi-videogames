...
const findgamebyname = async (name) => {
  ...
  if (findgame.length > 0) {
    ...
    const newgame = await fetch(
      `https://api.rawg.io/api/games?search=${name}&key=c58977baa48f48a2b4a0cb43a9436d91`
    );
    ...
    const allnewaas = arrayindex.map(async (index) => {
      ...
      const ides = allgeneros
        .filter((genre) => gamegenre.includes(genre.genre))
        .map((genre) => genre.id);
      const newaas = await Game.create({
        id: data.results[index].id.toString(),
        name: data.results[index].name,
        devices: data.results[index].platforms?.map(
